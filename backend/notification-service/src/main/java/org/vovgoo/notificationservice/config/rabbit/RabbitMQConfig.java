...
public class RabbitMQConfig {
    ...
    public Binding signupPhoneBinding(Queue signupPhoneQueue, TopicExchange userEventsExchange) {
        return BindingBuilder.bind(signupPhoneQueue).to(userEventsExchange).with(SIGNUP_PHONE_KEY);
    }

    @Bean
    public Binding phoneChangeBinding(Queue phoneChangeQueue, TopicExchange userEventsExchange) {
        return BindingBuilder.bind(phoneChangeQueue).to(userEventsExchange).with(PHONE_CHANGE_KEY);
    }

    @Bean
    public Binding emailChangeBinding(Queue emailChangeQueue, TopicExchange userEventsExchange) {
        return BindingBuilder.bind(emailChangeQueue).to(userEventsExchange).with(EMAIL_CHANGE_KEY);
    }
}
