...
import { DishCardAdminSkeleton } from "@/components/dish/DishCardAdminSkeleton";
...
export const AdminDishList: React.FC<AdminDishListProps> = ({ restaurant, isPending, pageSize = 6 }) => {
    ...
    <div className="flex flex-col gap-6">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {(isPending || isFetching || !restaurantId)
          ? Array(pageSize).fill(0).map((_, i) => <DishCardAdminSkeleton key={i} />)
          : data?.content.length
