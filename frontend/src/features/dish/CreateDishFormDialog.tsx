...
export const CreateDishFormDialog: React.FC<CreateDishFormDialogProps> = ({
  restaurant,
  isPending,
}) => {
  ...
  const onSubmit = (data: DishCreateFormData) => {
    const payload: DishCreateRequest = {
      ...
      portionInGrams:
        data.portionInGrams !== undefined
          ? parseFloat(Number(data.portionInGrams).toFixed(2))
          : undefined,
      proteins:
        data.proteins !== undefined ? parseFloat(Number(data.proteins).toFixed(2)) : undefined,
      fats: data.fats !== undefined ? parseFloat(Number(data.fats).toFixed(2)) : undefined,
      carbohydrates:
        data.carbohydrates !== undefined
          ? parseFloat(Number(data.carbohydrates).toFixed(2))
          : undefined,
      spicy: data.spicy,
      vegan: data.vegan,
    ...
    <AlertDialog open={open} onOpenChange={setOpen}>
      ...
      <AlertDialogContent className="sm:max-w-none w-[700px]">
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            Заполните форму ниже, чтобы добавить новое блюдо для ресторана{' '}
