...
export const UpdateDishForm: React.FC<UpdateDishFormProps> = ({
  dish,
  restaurant,
  isPending
}) => {
        ...
        <Form {...form}>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <FormField control={form.control} name="name" render={({ field }) => (
              <FormItem>
                <FormControl>
                  {isPending ? <Skeleton className="h-10 w-full mb-4 bg-gray-400" /> :
                    <TextInput name={field.name} control={form.control} placeholder="Название блюда" error={form.formState.errors.name?.message}/>}
                </FormControl>
              </FormItem>
            )}/>
            ...
            <FormField control={form.control} name="description" render={({ field }) => (
              <FormItem className="col-span-full">
                <FormControl>
                  {isPending ? <Skeleton className="h-24 w-full mb-4 bg-gray-400" /> :
                    <TextareaInput name={field.name} control={form.control} placeholder="Описание блюда" rows={4} error={form.formState.errors.description?.message}/>}
                </FormControl>
              </FormItem>
            )}/>
            ...
            <FormField control={form.control} name="portionInGrams" render={({ field }) => (
              <FormItem>
                <FormControl>
                  {isPending ? <Skeleton className="h-10 w-full mb-4 bg-gray-400" /> :
                    <NumberInput name={field.name} control={form.control} placeholder="Вес в граммах" error={form.formState.errors.portionInGrams?.message}/>}
                </FormControl>
              </FormItem>
            )}/>
