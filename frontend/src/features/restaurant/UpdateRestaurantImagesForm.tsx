...
import { type RestaurantResponse, type ImageResponse, restaurantService } from '@/api';
...
export const UpdateRestaurantImagesForm: React.FC<UpdateRestaurantImagesFormProps> = ({
  restaurant,
  isPending,
}) => {
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        <CardTitle>Галерея ресторана</CardTitle>
      ...
      <CardContent className="flex flex-col">
        ...
        {isPending ? (
          ...
          <div className="grid grid-cols-3 gap-4 mt-4">
            {restaurant?.images?.map((image: ImageResponse) => (
