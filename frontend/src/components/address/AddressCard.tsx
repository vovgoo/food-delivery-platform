  ...
  return (
    <Card
      className={`min-h-36 flex gap-4 p-4 border rounded-lg items-start text-sm ${
        address.isDefault ? 'bg-black text-white border-black' : 'bg-white'
      }`}
    >
      <CardContent className="p-0 flex-1 flex flex-col">
        <div className="flex items-center gap-2">
          <MapPinIcon
            className={`w-6 h-6 ${address.isDefault ? 'text-orange-500' : 'text-gray-400'}`}
          />
          <div className="font-medium text-sm">
            {address.country}, {address.zip}, {address.state}, {address.city}, {address.street},{' '}
            {address.house}
        ...
        {address.deliveryInstructions && (
          <div className="mt-5 text-sm">{address.deliveryInstructions}</div>
        )}
      </CardContent>
